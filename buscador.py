...
# --- HACK PARA O RENDER NÃO DERRUBAR O BOT ---
app = Flask('')
@app.route('/')
...
TERMOS_BUSCA = ["perfume masculino", "camiseta masculina ribana", "tenis nike masculino"]
...
def monitor():
    ...
    while True:
        print(f"⏰ {time.strftime('%H:%M:%S')} - Varrendo...")
        sacola = []
                        ...
                        try:
                            ...
                            if len(p_tags) >= 2:
                                ...
                                if nome not in historico:
                                    ...
                                    sacola.append({'foto': foto, 'msg': f"🔥 <b>PREÇO BAIXOU!</b>\n\n✅ {nome}\n\nDe R$ {historico[nome]:.2f} ❌\nPor R$ {p_new:.2f} ✅\n\n🔗 Link: {link}"})
                                    historico[nome] = p_new
                        except: continue
            except: continue
        ...
        if sacola:
            ...
            for item in sacola:
                requests.post(f"https://api.telegram.org/bot{TOKEN_TELEGRAM}/sendPhoto", data={"chat_id": CHAT_ID, "photo": item['foto'], "caption": item['msg'], "parse_mode": "HTML"})
                time.sleep(15)
...
if __name__ == "__main__":
    # Inicia o "site de fachada" e o monitor ao mesmo tempo
    t = Thread(target=run)
    t.start()
    monitor()